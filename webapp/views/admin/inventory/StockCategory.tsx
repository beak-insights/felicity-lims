...
const StockCategory = defineComponent({
    ...
    setup(props, ctx) {
        ...
        function addStockCategory(): void {
          const payload = { ...form }
          withClientMutation(ADD_STOCK_CATEGORY, { payload }, "createStockCategory")
        ...
        function editStockCategory(): void {
          ...
          withClientMutation(EDIT_STOCK_CATEGORY, { uid: form.uid, payload }, "updateStockCategory")
    ...
    render() {
        return (
            ...
            <button onClick={() => this.FormManager(true, null)}
               class="px-2 py-1 border-sky-800 border text-sky-800rounded-smtransition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Add Stock Category</button>
            <hr />
                        ...
                        return (
                          <tr key={category?.uid}>
                            ...
                            <td class="px-1 py-1 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                                <button onClick={() => this.FormManager(false, category)} class="px-2 py-1 mr-2 border-sky-800 border text-sky-800 rounded-sm transition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Edit</button>
                            </td>
                          </tr>
                        )
                      })}

...
const StockPackaging = defineComponent({
    ...
    setup(props, ctx) {
        ...
        function addStockPackaging(): void {
          const payload = { ...form }
          withClientMutation(ADD_STOCK_PACKAGING, { payload }, "createStockPackaging")
          .then((result) => inventoryStore.addPackaging(result));
        }
        ...
        function editStockPackaging(): void {
          ...
          withClientMutation(EDIT_STOCK_PACKAGING, { uid: form.uid, payload }, "updateStockPackaging")
          .then((result) => inventoryStore.updatePackaging(result));
        }
    ...
    render() {
        return (
            ...
            <button onClick={() => this.FormManager(true, null)}
               class="px-2 py-1 border-sky-800 border text-sky-800rounded-smtransition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Add Stock Packaging</button>
            <hr />
            ...
            <div class="overflow-x-auto mt-4">
                ...
                <table class="min-w-full">
                    <thead>
                    <tr>
                        <th class="px-1 py-1 border-b-2 border-gray-300 text-left text-sm leading-4 text-gray-800 tracking-wider">Packaging Name</th>
                        ...
                        return (
                          <tr key={pckg?.uid}>
                            ...
                            <td class="px-1 py-1 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                                <button onClick={() => this.FormManager(false, pckg)} class="px-2 py-1 mr-2 border-sky-800 border text-sky-800 rounded-sm transition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Edit</button>
                            </td>
                          </tr>
                        )
                      })}

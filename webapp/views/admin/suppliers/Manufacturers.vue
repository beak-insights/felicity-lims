    ...
    <div class="container w-full my-4">
        <hr>
          <button @click="FormManager(true)"
           class="px-2 py-1 border-primary border text-primary rounded-sm transition duration-300 hover:bg-primary hover:text-primary-foreground focus:outline-none">Add Manufacturer</button>
        <hr>
            ...
            <table class="min-w-full">
                ...
                <tr v-for="manufacturer in manufacturers" :key="manufacturer?.uid">
                    ...
                    <div class="flex items-center">
                        <div>
                        <div class="text-sm leading-5 text-foreground">{{ manufacturer?.name }}</div>
                        </div>
                    </div>
                    </td>
                    <td class="px-1 py-1 whitespace-no-wrap border-b border-border">
                    ...
                    <td class="px-1 py-1 whitespace-no-wrap text-right border-b border-border text-sm leading-5">
                        <button @click="FormManager(false, manufacturer)" class="px-2 py-1 mr-2 border-primary border text-primary rounded-sm transition duration-300 hover:bg-primary hover:text-primary-foreground focus:outline-none">Edit</button>
                    </td>
                </tr>
                </tbody>
            </table>

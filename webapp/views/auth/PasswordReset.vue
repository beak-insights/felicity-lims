      ...
      <div class="bg-background/95 backdrop-blur-sm rounded-xl shadow-2xl p-8 space-y-6">
        ...
        <div class="flex flex-col items-center space-y-2">
          <div class="relative w-16 h-16 flex items-center justify-center">
            <div class="absolute w-12 h-12 bg-accent rounded-xl flex items-center justify-center transform rotate-45">
              <svg class="w-8 h-8 text-primary-foreground transform -rotate-45" viewBox="0 0 512 512" fill="none">
                <path
        ...
        <form class="space-y-5" @submit.prevent="submitForm" v-if="!auth.resetData?.canReset">
          <div v-if="!auth.receivedToken">
            ...
            <div class="mb-6">
              <button
                type="button"
                @click="emit('forgot')"
                class="inline-flex items-center text-sm text-accent hover:text-indigo-800 transition-colors duration-200"
              >
            ...
            <button
              ...
              :class="[
                ...
                email ? 'bg-accent hover:bg-accent' : 'bg-muted cursor-not-allowed'
              ]"
            >
          ...
          <div v-else class="space-y-5">
            ...
            <button
              ...
              :class="[
                'w-full px-4 py-2 rounded-lg text-primary-foreground font-medium transition-colors duration-200',
                token ? 'bg-accent hover:bg-accent' : 'bg-muted cursor-not-allowed'
              ]"
            >
              Submit Token
            </button>
        ...
        <form v-else class="space-y-5" @submit.prevent="changePassword">
          <div class="text-center mb-4">
            <p class="text-foreground">
              Hello <span class="bg-secondary px-2 py-1 rounded">@{{ auth?.resetData?.username }}</span>
            </p>
            <p class="text-sm text-foreground mt-1">Please enter your new password below</p>
          </div>
          ...
          <button
            ...
            :class="[
              'w-full px-4 py-2 rounded-lg text-primary-foreground font-medium transition-colors duration-200',
              formIsValid ? 'bg-accent hover:bg-accent' : 'bg-muted cursor-not-allowed'
            ]"
          >
            Reset Password
          </button>

...
class FelicityBarCoder:
    def __init__(self, page_width=40.0, page_height=30.0, barcode_width=30, barcode_height=7.5):
    ...
    async def _make(self, data: list[BarCode]):
        for _meta in data:
            ...
            Code128(_meta.barcode, writer=ImageWriter()).write(svg_img_bytes, options={"write_txt": False})
            with NamedTemporaryFile(delete=False, suffix='.png') as temp:
                temp.write(svg_img_bytes.getvalue())
                temp_path = temp.name
            self.pdf.image(temp_path, x=self.margin_left, y=self.margin_top, w=self.barcode_width,

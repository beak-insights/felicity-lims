...
from felicity.apps.analysis.models.analysis import QCSet, Sample
...
class CQSetWorkFlow:
    ...
    async def cancel(cls, uid):
        ...
        await qc_set.change_status(States.Sample.CANCELLED)

    @classmethod
    async def submit(cls, uid):
        qc_set = await QCSet.get(uid=uid)
        await cls._guard(qc_set, "submit")
        await qc_set.change_status(States.Sample.AWAITING)

    @classmethod
    async def approve(cls, uid):
        qc_set = await QCSet.get(uid=uid)
        await cls._guard(qc_set, "approve")
        await qc_set.change_status(States.Sample.APPROVED)
    ...
    async def _guard(qc_set: QCSet, action) -> bool:
        ...
        if action == "submit":
            allow = all(list(map(lambda s: s.status == States.Sample.AWAITING, samples)))
        elif action == "approve":
            allow = all(list(map(lambda s: s.status == States.Sample.APPROVED, samples)))
        elif action == "cancel":
            allow = all(list(map(lambda s: s.status == States.Sample.CANCELLED, samples)))
        else:
            ...

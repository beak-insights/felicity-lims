...
async def bill_order(analysis_request: a_entities.AnalysisRequest, auto_bill=False):
    ...
    bill = await TestBillService().create(bill_in)
    ...
    for t_in in in_transactions:
        ...
        await TestBillService().update(bill.uid, bill_update_in)
        # update transaction
...
async def apply_voucher(
    ...
    code: VoucherCode = await VoucherCodeService().get(code=voucher_code)
    ...
    voucher: Voucher = await VoucherService().get(uid=code.voucher_uid)
    ...
    for tras_in in in_trans:
        transaction = await TestBillTransactionService().create(tras_in)
        # apply sale discounts from the transaction
        bill_update_in = TestBillUpdate(total_paid=bill.total_paid + transaction.amount)
        bill = await TestBillService().update(bill.uid, bill_update_in)
        # update transaction
        tra_update_in = TestBillTransactionUpdate(
            processed=True,
            is_success=True,

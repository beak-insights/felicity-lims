...
from apps.billing.config import (
    DiscountType, DiscountValueType, TransactionType
)
...
async def bill_order(analysis_request: a_models.AnalysisRequest, auto=False):
    ...
    if auto and not lab_settings.allow_auto_billing:
        return
    
    # calculate bill and marshall prices for future reference
    total_charged = 0.00
    for sample in analysis_request.samples:
        profile_uids = list(map(lambda p: p.uid, sample.profiles))
        analysis_uids = list(map(lambda a: a.uid, sample.analyses))
    ...
    for p_price in profiles_prices:
        ...
        if p_discount:
            ...
            in_transactions.append(TestBillTransactionCreate(
                test_bill_uid=None,
                kind=TransactionType.AUTO_DISCOUNT,
    ...
    for t_in in  in_transactions:
        # add a transaction
        t_in.test_bill_uid = bill.uid
        transaction = await TestBillTransaction.create(t_in)

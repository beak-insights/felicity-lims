...
class ReflexRule(Auditable):
    ...
    async def update(
        self, obj_in: dict | schemas.ReflexRuleUpdate
    ) -> schemas.ReflexRule:
...
class ReflexBrain(Auditable):
    ...
    @classmethod
    async def create(
        cls, obj_in: dict | schemas.ReflexBrainCreate
    ) -> schemas.ReflexBrain:
        data = cls._import(obj_in)
        return await super().create(**data)

    async def update(
        self, obj_in: dict | schemas.ReflexBrainUpdate
    ) -> schemas.ReflexBrain:
        data = self._import(obj_in)
        return await super().update(**data)
...
class ReflexAction(Auditable):
    ...
    @classmethod
    async def create(
        cls, obj_in: dict | schemas.ReflexActionCreate
    ) -> schemas.ReflexAction:
        data = cls._import(obj_in)
        return await super().create(**data)

    async def update(
        self, obj_in: dict | schemas.ReflexActionUpdate
    ) -> schemas.ReflexAction:
        data = self._import(obj_in)
        return await super().update(**data)

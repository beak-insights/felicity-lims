...
class ShipmentService(BaseService[Shipment, ShipmentCreate, ShipmentUpdate]):
    ...
    async def get_samples(self, uid: str):
        return list(
            map(
                lambda ss: ss.sample, await self.shipped_sample_service.get_all(shipment_uid=uid)
            )
        )

    async def change_state(self, uid: str, state, updated_by_uid):
        return await super().update(uid, {"state": state, "updated_by_uid": updated_by_uid})

    async def finalise(self, uid: str, finaliser):
        if self.state == ShipmentState.PREPERATION:
            saved = await super().update(uid, {"state": ShipmentState.READY, "updated_by_uid": finaliser.uid})
    ...
    async def dispatch(self, uid: str, dispatcher):
        if self.state == ShipmentState.READY:
            ...
            saved = await super().update(uid, {"state": ShipmentState.READY, "dispatched_by_uid": dispatcher.uid})

...
from domain.exceptions import NotAllowedError, AleadyExistsError, ValidationError
...
async def test_add_user(user_service, mocker, user_data):
    ...
    mocker.patch.object(user_service.repository, "create", return_value=mocker.AsyncMock(return_value=User(**{"uid": uid, **user_data})))
    
    result = await user_service.add_user(**user_data)
    
    user_service.repository.get.assert_called_once_with(email=user_data["email"])
...
async def test_add_user_email_already_exists(user_service, mocker, user_data):
    ...
    with pytest.raises(AleadyExistsError):

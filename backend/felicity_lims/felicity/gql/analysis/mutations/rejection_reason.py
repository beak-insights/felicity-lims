...
RejectionReasonResponse = strawberry.union("RejectionReasonResponse",
...
@strawberry.mutation
async def update_rejection_reason(info, uid: int, reason: str) -> RejectionReasonResponse:
    is_authenticated, felicity_user = await auth_from_info(info)
    ...
    if not rejection_reason:
        return OperationError(
            error=f"rejection reason with uid {uid} does not exist"
        )

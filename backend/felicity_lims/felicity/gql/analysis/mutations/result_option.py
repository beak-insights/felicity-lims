...
ResultOptionResponse = strawberry.union("ResultOptionResponse",
...
@strawberry.mutation
async def create_result_option(info, payload: ResultOptionInputType) -> ResultOptionResponse:
    ...
    if not payload.analysis_uid:
        return OperationError(
            error="Analysis to attach Result Option Required"
        )

    if not payload.option_key or not payload.value:
        return OperationError(
            error="Result option key and value Required"
        )
...
@strawberry.mutation
async def update_result_option(info, uid: int, payload: ResultOptionInputType) -> ResultOptionResponse:
    ...
    if not result_option:
        return OperationError(
            error=f"ResultOption with uid {uid} does not exist"
        )

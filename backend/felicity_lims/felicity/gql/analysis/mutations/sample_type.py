...
SampleTypeResponse = strawberry.union("ResultOptionResponse",
...
async def create_sample_type(info, payload: SampleTypeInputType) -> SampleTypeResponse:
    ...
    if exists:
        return OperationError(
            error=f"Sample Type: {payload.name} already exists"
        )
...
@strawberry.mutation
async def update_sample_type(info, uid: int, payload: SampleTypeInputType) -> SampleTypeResponse:
    ...
    if not sample_type:
        return OperationError(
            error=f"Sample type with uid {uid} does not exist"
        )

...
from felicity.gql.kanban.types import (
    BoardType, BoardCursorPage, BoardEdge,
    BoardListingType,
    ListingTaskType,
    TaskMilestoneType,
    TaskCommentType,
...
class KanBanQuery:
    @strawberry.field
    async def board_all(self, info, page_size: Optional[int] = None,
                        after_cursor: Optional[str] = None, before_cursor: Optional[str] = None,
                        text: Optional[str] = None, sort_by: Optional[List[str]] = None) -> BoardCursorPage:
        filters = {}

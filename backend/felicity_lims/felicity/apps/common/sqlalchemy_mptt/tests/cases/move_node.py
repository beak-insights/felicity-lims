...
class MoveBefore(object):
    ...
    def test_move_one_tree_before_another(self):
        ...
        node = (
            ...
            .filter(self.model.get_pk_column() == 12)
            .one()
        )
        node.move_before("1")
        _level = node.get_default_level()
    ...
    def test_move_before_function(self):
        ...
        """
        node = (
            self.session.query(self.model).filter(self.model.get_pk_column() == 8).one()
        )
        node.move_before("4")
        _level = node.get_default_level()
        self.assertEqual(
    ...
    def test_move_one_tree_before_other_tree(self):
        self.fixture.add(self.model, os.path.join("fixtures", "tree_3.json"))
        self.session.commit()
        self.maxDiff = None
        ...
        node = (
            self.session.query(self.model)
            .filter(self.model.get_pk_column() == 23)
            .one()
        )
        node.move_before("1")
...
class MoveAfter(object):
    ...
    def test_move_to_toplevel_where_much_trees_from_right_side(self):
        ...
        node = (
            self.session.query(self.model)
            .filter(self.model.get_pk_column() == 15)
            .one()
        )
        node.move_after("1")
        _level = node.get_default_level()
        ...
        node = (
            self.session.query(self.model)
            .filter(self.model.get_pk_column() == 20)
            .one()
        )
        node.move_after("1")
        """ level           tree_id = 1
...
class MoveInside(object):
    ...
    def test_move_inside_to_the_same_parent_function(self):
        ...
        """
        node = (
            self.session.query(self.model).filter(self.model.get_pk_column() == 6).one()
        )
        node.move_inside("4")
        _level = node.get_default_level()
        self.assertEqual(

...
class DataIntegrity(object):
    ...
    def test_greatest_right_is_always_double_number_of_nodes(self):
        ...
        table = self.model
        result = (
            self.session.query(func.count(table.get_pk_name()), func.max(table.right))
            .group_by(table.tree_id)
            .all()
        )
        for tree in result:
    ...
    def test_hierarchy_structure(self):
        ...
        pivot = (
            ...
            .filter(table.right - table.left != 1)
            .filter(table.parent_id != None)
            .first()
        )  # noqa
        ...
        descendants = (
            self.session.query(table).filter(table.is_descendant_of(pivot)).all()
        )
        ...
        ancestors = (
            self.session.query(table)
            .filter(table.is_ancestor_of(pivot, inclusive=True))
            .all()
        )
        for ancestor in ancestors:
        ...
        descendants = (
            self.session.query(table)
            .filter(table.is_descendant_of(pivot, inclusive=True))
            .all()
        )
        for descendant in descendants:

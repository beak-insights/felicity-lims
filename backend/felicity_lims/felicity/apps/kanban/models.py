...
class BoardListing(BaseAuditDBModel):
    ...
    listing_tasks = relationship(
        "ListingTask", back_populates="listing", lazy="selectin"
    )
...
class ListingTask(BaseAuditDBModel):
    ...
    listing = relationship(
        BoardListing, back_populates="listing_tasks", lazy="selectin"
    )
...
class TaskMilestone(BaseAuditDBModel):
    ...
    assignee = relationship(
        User, foreign_keys=[assignee_uid], backref="tasks_milestones", lazy="selectin"
    )
    ...
    async def update(
        self, obj_in: schemas.TaskMilestoneUpdate
    ) -> schemas.TaskMilestone:
        data = self._import(obj_in)
        return await super().update(**data)

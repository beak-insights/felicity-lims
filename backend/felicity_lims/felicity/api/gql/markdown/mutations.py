...
DocumentResponse = strawberry.union(
    "DocumentResponse", (types.DocumentType, OperationError), description=""  # noqa
)
...
class MarkdownMutations:
    ...
    async def create_document_tag(self, info, name: str) -> DocumentTagResponse:
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can create document tags",
        )

        if not name:
            return OperationError(error="Please Provide a Tag name")

        exists = await models.DocumentTag.get(name=name)
        if exists:
            return OperationError(error=f"DocumentTag {name} already exists")
    ...
    async def update_document_tag(
        self, info, uid: int, name: Optional[str] = None
    ) -> DocumentTagResponse:  # noqa
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can update document tags",
        )
    ...
    async def create_document_category(
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can create document categories",
        )

        if not name:
            return OperationError(error="Please Provide a category name")

        exists = await models.DocumentCategory.get(name=name)
        if exists:
            return OperationError(error=f"Document Category {name} already exists")
    ...
    async def update_document_category(
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can update document categories",
        )
    ...
    async def create_document(
        self, info, payload: MarkdownInputType
    ) -> DocumentResponse:  # noqa

        is_authenticated, felicity_user = await auth_from_info(info)
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can create document",
        )

        if not payload.name:
            return OperationError(error="Please Provide a name")
    ...
    async def update_document(
        root, info, uid: int, payload: MarkdownInputType
    ) -> DocumentResponse:  # noqa
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can update document",
        )

...
BoardResponse = strawberry.union(
    "BoardResponse", (types.BoardType, OperationError), description=""  # noqa
)

BoardListingResponse = strawberry.union(
    "BoardListingResponse",
    (types.BoardListingType, OperationError),  # noqa
    description="",
)

ListingTaskResponse = strawberry.union(
    "ListingTaskResponse",
    (types.ListingTaskType, OperationError),  # noqa
    description="",
)

TaskMilestoneResponse = strawberry.union(
    "TaskMilestoneResponse",
    (types.TaskMilestoneType, OperationError),  # noqa
    description="",
)

TaskCommentResponse = strawberry.union(
    "TaskCommentResponse",
...
class KanBanMutations:
    ...
    async def create_board(self, info, payload: BoardInputType) -> BoardResponse:
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can create kanban boards",
        )

        if not payload.title:
            return OperationError(error="Please Provide a Board title")

        exists = await models.Board.get(title=payload.title)
        if exists:
            return OperationError(error=f"Board {payload.title} already exists")
    ...
    async def update_board(
        self, info, uid: int, payload: BoardInputType
    ) -> BoardResponse:
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can update kanban boards",
        )
    ...
    async def delete_board(self, info, uid: int) -> DeleteResponse:
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can delete kanban board",
        )
    ...
    @strawberry.mutation
    async def create_board_listing(
        self, info, payload: BoardListingInputType
    ) -> BoardListingResponse:
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can create  kanban board listings",
        )

        if not payload.title:
            return OperationError(error="Please Provide a Listing title")

        exists = await models.BoardListing.get(
            title=payload.title, board_uid=payload.board_uid
        )
        if exists:
            return OperationError(
    ...
    @strawberry.mutation
    async def update_board_listing(
        self, info, uid: int, payload: BoardListingInputType
    ) -> BoardListingResponse:
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can update kanban board listings",
        )
    ...
    async def delete_board_listing(self, info, uid: int) -> DeleteResponse:
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can delete kanban board listings",
        )
    ...
    async def create_listing_task(
        self, info, title: str, listing_uid: int, description: Optional[str] = None
    ) -> ListingTaskResponse:
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can create kanban tasks",
        )

        if not title:
            return OperationError(error="Please Provide a Task title")
    ...
    async def update_listing_task(
        self, info, uid: int, payload: ListingTaskInputType
    ) -> ListingTaskResponse:
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can update kanban tasks",
        )
    ...
    async def delete_listing_task(self, info, uid: int) -> DeleteResponse:
        ...
        verify_user_auth(
            is_authenticated,
            felicity_user,
            "Only Authenticated user can delete kanban tasks",
        )
    ...
    @strawberry.mutation
    async def duplicate_listing_task(
        self, info, uid: int, title: str
    ) -> ListingTaskResponse:
    ...
    async def create_task_comment(
        self, info, comment: str, task_uid: int
    ) -> TaskCommentResponse:
        ...
        if not task_uid:
            return OperationError(error="Comment Missing the associated Listing Task")

        if not comment:
            return OperationError(error="Task Comment cannot be blank")
    ...
    @strawberry.mutation
    async def create_task_milestone(
        self,
        info,
        title: str,
        task_uid: int,
        ...
        if not task_uid:
            return OperationError(error="Milestone Missing the associated Listing Task")

        if not title:
            return OperationError(error="Task Comment title cannot be blank")

...
<script setup lang="ts">
  ...
  function addDocument(): void {
    ...
    withClientMutation(ADD_MARKDOWN_DOCUMENT, { payload }, "createDocument")
    .then((result) => documentStore.addDocument(result));
  }


  function editDocument(): void {
    const payload = { name: form.name, departmentUid: form.departmentUid }
    withClientMutation(EDIT_MARKDOWN_DOCUMENT, { uid: form.uid, payload }, "updateDocument")
    .then((result) => documentStore.updateDocument(result));
  }

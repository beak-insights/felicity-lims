...
import modal from '../modals/simpleModal.vue'
import { ClientContact,  } from '../../../store/modules/clients';
import { ADD_CLIENT_CONTACT } from '../../../graphql/clients.mutations';

export const IClientContact = typeof ClientContac;
export default {
    name: "tab-contacts",
    components: {
        modal,
    ...
    setup(props){
        ...
        let store = useStore();
        const nullClientContact = new ClientContact();
        let contact = ref(nullClientContact);

        const { executeMutation: createClientContact } = useMutation(ADD_CLIENT_CONTACT);

        let resetContact = () => Object.assign(contact, { ...nullClientContact });

        function addClientContact() {
          createClientContact({clientUid: '1', firstName: contact.value.firstName}).then((result) => {
            Object.assign(contact, result);
          });
        }

        function editClientContact() {
          updateClientContact({ uid: form.uid, name: form.name, code: form.code, active: true }).then(
            (result) => {
              Object.assign(contact, result);
            },
          );
        ...
        function saveForm() {
            ...
            // if (!createItem.value === true) updateClient();

...
const StockItem = defineComponent({
    ...
    setup(props, ctx) {
        ...
        function addStockItem(): void {
          const payload = { ...form }
          withClientMutation(ADD_STOCK_ITEM, { payload }, "createStockItem")
          .then((result) => inventoryStore.addItem(result));
        }
        ...
        function editStockItem(): void {
          ...
          withClientMutation(EDIT_STOCK_ITEM, { uid: form.uid, payload }, "updateStockItem")
          .then((result) => inventoryStore.updateItem(result));
        }
    ...
    render() {
        ...
        return (
            ...
            <hr />
            <button onClick={() => this.FormManager(true, null)}
               class="px-2 py-1 border-sky-800 border text-sky-800rounded-smtransition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Add Stock Item</button>
            <hr />
            ...
            <div class="overflow-x-auto mt-4">
                ...
                <table class="min-w-full">
                    <thead>
                    <tr>
                        <th class="px-1 py-1 border-b-2 border-gray-300 text-left text-sm leading-4 text-gray-800 tracking-wider">Item Name</th>
                        <th class="px-1 py-1 border-b-2 border-gray-300 text-left text-sm leading-4 text-gray-800 tracking-wider">Description</th>
                        <th class="px-1 py-1 border-b-2 border-gray-300 text-left text-sm leading-4 text-gray-800 tracking-wider">Main User (Department)</th>
                        ...
                        return (
                          <tr key={item?.uid}>
                            ...
                            <td class="px-1 py-1 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                                <button onClick={() => this.FormManager(false, item)} class="px-2 py-1 mr-2 border-sky-800 border text-sky-800 rounded-sm transition duration-300 hover:bg-sky-800 hover:text-white focus:outline-none">Edit</button>
                            </td>
                          </tr>
                        )
                      })}
                    ...
                    <div class="grid grid-cols-2 gap-x-4 mb-4">
                      ...
                      <label class="block col-span-2 mb-2">
                        ...
                        <label class="block col-span-1 mb-2">
                            ...
                            <option></option>
                            {this.departments.map(department => (<option key={department.uid} value={department?.uid}>{ department.name }</option>))}
                            </select>
                        </label>
                    </div>
                    <hr />

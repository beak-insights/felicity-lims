<script setup lang="ts">
    ...
    if(target?.value==='client-samples') sampleStore.fetchAnalysisRequestsForClient(targetUid?.value);
    ...
    watch(() => props.targetUid, (uid, prev) => {
        ...
        if(target?.value==='client-samples') sampleStore.fetchAnalysisRequestsForClient(uid);
    })
...
<template>
    <div class="overflow-x-auto mt-4">
        <div class="align-middle inline-block min-w-full shadow overflow-hidden bg-white shadow-dashboard p-2 rounded-bl-sm rounded-br-sm">
            <table class="min-w-full">
                ...
                <tbody class="bg-white" v-for="request in analysisRequests" :key="request.uid">
                        ...
                        <tr v-for="sample in request.samples" :key="sample.uid" >
                            ...
                            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
                            <div class="text-sm leading-5 text-sky-800">{{ request.patient?.firstName }} {{ request.patient?.lastName }}</div>
                            </td>
                            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
                            <div class="text-sm leading-5 text-sky-800">{{ request.patient?.clientPatientId }}</div>
                            </td>
                            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
                            ...
                            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
                            <button type="button" class="bg-cyan-600 text-white p-1 rounded-sm leading-none">{{ sample.status }}</button>
                            </td>
                            <td class="px-1 py-1 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
                                <router-link 
                                :to="{ name: 'sample-detail', 

...
const patientCount = computed(
  () =>
    patientStore.getPatients?.length + " of " + patientStore.getPatientCount + " patients"
);
...
function addPatient() {
  withClientMutation(
    ADD_PATIENT,
    {
      clientPatientId: patientForm.clientPatientId,
      ...
      ageDobEstimated: patientForm.ageDobEstimated,
      clientUid: patientForm.clientUid,
      phoneMobile: patientForm.phoneMobile,
      consentSms: patientForm.consentSms,
    },
    "createPatient"
    ...
    >
      <table class="min-w-full">
        ...
        <tbody class="bg-white">
          <tr v-for="pt in patients" :key="pt.patientId">
            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
              <router-link
                :to="{ name: 'patient-detail', params: { patientUid: pt?.uid } }"
            ...
            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
              <router-link
                :to="{ name: 'patient-detail', params: { patientUid: pt?.uid } }"
                class="p-1 ml-2 border-white border text-gray-500rounded-smtransition duration-300 hover:border-sky-800 hover:text-sky-800 focus:outline-none"
                >{{ getPatientFullName(pt) }}</router-link
              >
            </td>
            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
            ...
            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
              <div class="text-sm leading-5 text-sky-800">{{ pt.gender }}</div>
            </td>
            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
            ...
            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
              <div class="text-sm leading-5 text-sky-800">
                {{ pt?.client?.district?.province?.name }}
              </div>
            </td>
            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
              <div class="text-sm leading-5 text-sky-800">
                {{ pt?.client?.district?.name }}
              </div>
            </td>
            <td class="px-1 py-1 whitespace-no-wrap border-b border-gray-500">
    ...
    <section class="flex justify-between">
      ...
      <div class="my-4 flex sm:flex-row flex-col">
        ...
        <div class="flex flex-row mb-1 sm:mb-0">
          <div class="relative">
            <select
              class="appearance-none h-full rounded-sm border block w-full bg-white border-gray-400 text-gray-700 py-2 px-4 pr-8 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
              v-model="patientBatch"
